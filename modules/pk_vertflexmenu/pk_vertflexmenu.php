...
class pk_vertflexmenu extends Module {
	...
	public function displayAddTabForm($id_menu = NULL) {
		...
		$id_shop = (int)Shop::getContextShopID();	
		$id_lang = (int)Context::getContext()->language->id;	
		$this->context->controller->addJS(($this->_path) . 'js/itemmanager-vert.js');
			...
			switch ($type)
			{
				...
				case 'MAN':
					...
					if (!is_null($manufacturer->id))
					{
						...
						$link = new Link;
						$this->_menu .= '<li'.$selected.' class="menu-manufacturer"><a href="'.$link->getManufacturerLink((int)$id, $manufacturer->link_rewrite).'"><img src="'.$this->context->shop->getBaseURL().'img/m/'.$manufacturer->id.'-manu_alysum.jpg" alt="'.htmlspecialchars($manufacturer->name).'" title="'.htmlspecialchars($manufacturer->name).'" width="168" height="100" /></a></li>'.PHP_EOL;
					}
					break;
	...
	{	
		...
		foreach ($data as $link) {			
			...
			$this->_menu .= '<li class="v-flexmenuitem id_menu'.$link['id_pk_vertflexmenu_menus'].(($submenu) ? ' hasChildren' : '').(($link['narrow'] == 1)  ? ' narrow' : '').(($this->curPath() == $itemData['link']) ? ' current-item' : '').'" data-menuid="'.$link['id_pk_vertflexmenu_menus'].'"><a class="menu-title" href="'.$itemData['link'].'" '.(((!empty($itemData['new_window'])) && ($itemData['new_window'] == 1)) ? ' target="_blank"' : '').'>'.$icon_html.'<span>'.$itemData['name'].'</span></a>'.(($submenu) ? '<a href="#" class="opener"><svg class="svgic svgic-account"><use xlink:href="#si-arrowdown"></use></svg></a>' : '').'';		

...
class VertFlexMenuItem
{
	...
	public static function updateLink($link, $labels, $newWindow = 0, $id_shop, $id_link) {
		...
		Db::getInstance()->delete('pk_vertflexmenu_links_lang', 'id_pk_flexmenu_links = ' . (int)$id_link);
		foreach ($labels as $id_lang => $label) {
			Db::getInstance()->insert('pk_vertflexmenu_links_lang', array('id_pk_flexmenu_links' => (int)$id_link, 'id_lang' => (int)$id_lang, 'id_shop' => (int)$id_shop, 'label' => pSQL($label), 'link' => pSQL($link[$id_lang])));
		}
	}

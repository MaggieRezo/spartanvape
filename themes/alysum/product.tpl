			...
			{if isset($comments) && ($theme_settings.product_rating == 1)}
				...
				<div class="comment" id="product_comment">
					...
					{section name="i" start=0 loop=5 step=1}
						...
						{/if}
					{/section}
						<span class="hidden" itemprop="ratingValue">{$productGrade}</span>
						<span class="hidden" itemprop="reviewCount">{$totalVotes}</span>
						<span class="hidden" itemprop="itemReviewed" itemscope itemtype="http://schema.org/Product"><span itemprop="name">{$product->name|escape:'html':'UTF-8'}</span></span>
					</div>
				</div>		
			{/if}
